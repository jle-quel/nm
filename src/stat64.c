...
static inline struct section_64	*get_section(t_object const *object, struct segment_command_64 const *segment, uint32_t const index)
{
	if ((void *)segment + sizeof(struct segment_command_64) + (sizeof(struct section_64) * index) >= object->eof)
		return error(CORRUPTED_FILE, object->name);

	return (void *)segment + sizeof(struct segment_command_64) + (sizeof(struct section_64) * index);
}

static void			get_stat(t_object const *object, t_stat *stat, struct load_command const *lc, uint32_t const filetype)
{
	struct segment_command_64 const	*segment = (struct segment_command_64 const *)lc;
