...
static t_symbol	*get_symbol(struct nlist const symbol_table, char const *string_table, t_stat const *stat)
{
...
{
	struct nlist const		*symbol_table = (struct nlist const *)((void *)object->ptr + symtab->symoff);
...
{
	struct mach_header const	*header = (struct mach_header const *)object->ptr;
	struct load_command			*lc = NULL;
	struct symtab_command		*symtab = NULL;
	t_stat						*stat = NULL;
	t_tree						*tree = NULL;

	if ((lc = get_load_command(object, sizeof(struct mach_header))) == NULL)
	{
		title(object);
		return ;
	}
	...
	if ((stat = get_stat32(object, lc, header)) == NULL)
	{
		title(object);
		return ;
	}
